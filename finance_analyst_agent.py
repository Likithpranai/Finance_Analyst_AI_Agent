...
class FinanceAnalystReActAgent:
    ...
    def extract_stock_symbol(self, query):
        ...
        common_words = ['I', 'A', 'THE', 'FOR', 'AND', 'OR', 'RSI', 'MACD', 'EMA', 'SMA']
    ...
    def determine_tools_needed(self, query):
        ...
        # First, always get basic price information
        ...
        if any(term in query for term in ["info", "information", "about", "company", "details", "fundamentals"]):
            tools_needed.append("get_company_info")
        
        # Check for news needs
        if any(term in query for term in ["news", "headlines", "articles", "press", "announcement"]):
            tools_needed.append("get_stock_news")
        
        # Check for visualization needs
        if any(term in query for term in ["chart", "graph", "plot", "visual", "picture", "show me"]):
            tools_needed.append("visualize_stock")
        
        # If nothing specific was detected, get price and company info as default
        if not tools_needed:
    ...
    def process_query(self, query):
        ...
        try:
            # REASON: Extract stock symbol and determine which tools are needed
            symbol = self.extract_stock_symbol(query)
            tools_needed = self.determine_tools_needed(query)
            
            print(f"\nREACT - REASON: Analyzing query for {symbol}")
            ...
            for tool_name in tools_needed:
                ...
                # Handle special parameters for certain tools
            ...
            combined_result = f"Financial Analysis for {symbol}:\n\n"
            ...
            if model is not None:
                try:
                    ...
                    return self._generate_simple_analysis(symbol, results)
            else:
    ...
    def _generate_simple_analysis(self, symbol, results):
        """Generate a simple analysis when Gemini is not available"""
        analysis = f"Financial Analysis for {symbol}:\n\n"
        ...
        # Technical Analysis
        if "calculate_rsi" in results or "calculate_macd" in results:
            analysis += "\nTECHNICAL ANALYSIS\n-----------------\n"
